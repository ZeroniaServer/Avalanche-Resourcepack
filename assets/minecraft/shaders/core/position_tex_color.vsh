...
void main() {
	...
	// the vertex renders a bossbar, offset it
	if (check_alpha(alpha, 1.0)) {
		gl_Position = ProjMat * ModelViewMat * vec4(Position + vec3(0.0, -19.0, 0), 1.0);
	}
	else if (check_alpha(alpha, 2.0)) {
		gl_Position = ProjMat * ModelViewMat * vec4(Position + vec3(0.0, -38.0, 0), 1.0);
	}
	// END CUSTOM CODE
}