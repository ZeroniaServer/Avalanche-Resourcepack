...
#> Kill the hitbox
execute as @e[type=interaction,tag=GiftboxInteraction] at @s if block ~ ~-1 ~ air unless predicate game:low_arena run particle item{item:{id:"minecraft:diamond_hoe",components:{custom_model_data:25}}} ~ ~0.2 ~ 0.1 0.1 0.1 0.03 30 force
