...
item replace entity @s[tag=BlockySleigh,scores={CmdData=10}] contents with diamond_hoe[item_model="minecraft:weapons/blockysleigh"]
execute at @s[tag=BlockySleigh,scores={CmdData=10}] run playsound sleightransform master @a ~ ~ ~ 1 2
execute at @s[tag=BlockySleigh,scores={CmdData=10}] run playsound sleightransform master @a ~ ~ ~ 1 0
