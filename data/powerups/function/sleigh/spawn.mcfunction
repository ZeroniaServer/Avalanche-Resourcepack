...
$execute positioned ^ ^ ^-.1875 positioned ~ ~0.4171875 ~ run summon bamboo_raft ~ ~-0.5 ~ {Tags:["sleighboat","init"],Rotation:[$(rotX)f,0.0f],width:0,height:-0.5,Passengers:[{id:"marker"}],Invulnerable:1b,Silent:1b}
...
ride @e[type=bamboo_raft,tag=init,limit=1] mount @e[type=interaction,tag=boatanchor,tag=init,limit=1]
execute as @e[type=bamboo_raft,tag=init,limit=1] run team join noCol
execute as @e[type=bamboo_raft,tag=init,limit=1] run function entityid:assign_tag
